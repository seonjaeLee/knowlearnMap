...
- [../surface/list-page-spec.md](../surface/list-page-spec.md) — 목록 **상단** PageHeader + `table-toolbar` + `search-area` (표 **위** 영역)
- [dev-guide-table-ui.md](./dev-guide-table-ui.md) — 개발자용: 표 복붙·div 클릭 등 실수 방지·체크리스트 · **섹션 6: `BasicTable` MUI 스택·푸터·페이지네이션**  
- [../foundation/layout-guideline.md](../foundation/layout-guideline.md) — 본문 폭·패딩·브레이크포인트  
- [../foundation/design-tokens.md](../foundation/design-tokens.md) — `:root` 변수 규칙  
...
실제 색·숫자의 단일 소스는 `src/index.css`의 `:root`이며, 이름 규칙은 [../foundation/design-tokens.md](../foundation/design-tokens.md)를 따릅니다.
...
- **행 내 아이콘 버튼 통일 (`src/styles/kl-table-icon-actions.css`, `main.jsx`에서 전역 로드)**  
  - **크기·간격(`index.css`)**: `--kl-table-action-icon-size`(아이콘 실크기, 기본 **16px**), `--kl-table-action-icon-gap`(편집·삭제 사이, 기본 **0**), `--kl-table-action-hit-size`(클릭 영역), `--kl-table-actions-col-min`(액션 열 최소 폭). 행 내 액션 SVG는 이 토큰으로 통일하고, Lucide 기본 24px에 의존하지 않는다.  
  ...
  - **승인·긍정**: `.kl-table-icon-btn--success` — `var(--color-icon-success)` / `var(--color-icon-success-emphasis)`(`index.css`); 호버는 danger와 동일하게 `color-mix` 연한 톤.  
...
- **채우기형 강조(예: Admin 역할)** 색: 가능하면 `--control-pill-active-*` 패턴과 [../foundation/design-tokens.md](../foundation/design-tokens.md)의 pill 안내를 참고.
...
- 페이지 상단 여백·폭은 [../foundation/layout-guideline.md](../foundation/layout-guideline.md)의 본문 패딩과 맞춘다.
