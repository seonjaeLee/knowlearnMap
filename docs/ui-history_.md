...
- **목적**: 테이블·관리 화면 목업 협업 시 산출물 형식을 통일하고 부담을 줄임
- **영향**: 문서만 추가; 앱 동작·스타일 변경 없음

#### 문서 변경
- `docs/mockup-guide.md` — 목적, 참고 문서 링크, 산출물 형식(피그마/캡처), 화면별·테이블 체크리스트, 피할 패턴
- `docs/ui-system-outline.md` — 연관 문서 표에 `mockup-guide.md` 링크 추가

### 196) 목업 가이드 표현 정리

- **목적**: 제목·설명에서 불필요한 표현 제거
- **영향**: 문서 문구만 수정

#### 문서 변경
- `docs/mockup-guide.md` — 제목에서 부적절한 수식 제거
- `docs/ui-system-outline.md` — `mockup-guide` 행 설명 간소화
- `docs/ui-history.md` — §195 제목·목적 문구 정리

### 197) 프롬프트 관리 리디자인 체크리스트 예시 문서 추가

- **목적**: §4 체크리스트를 채운 요청 예시를 별도 파일로 보관
- **영향**: 문서만 추가

#### 문서 변경
- `docs/mockup-checklist-prompt-management-redesign.md` — 작업 의도, 표 형식 체크리스트, 문서 구성 항목

### 198) 데이터 테이블 UI 규격 초안 (`data-table-spec.md`)

- **목적**: 관리 화면 목록의 밀도·헤더·행·툴바를 토큰 기준으로 통일하고, Figma wireframe 레퍼런스와 연결
- **영향**: 문서만 추가·연동; 앱 코드 변경 없음

#### 문서 변경
- `docs/data-table-spec.md` — 밀집/기본 밀도, 헤더·본문·호버, 액션·pill·하단 페이지네이션, 구현 주의, 연관 문서
- `docs/ui-system-outline.md` — 연관 문서 표에 `data-table-spec.md` 추가
- `docs/mockup-guide.md` — §5 끝에 `data-table-spec.md` 안내 문장 추가
