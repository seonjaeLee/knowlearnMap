...
## 10. 구현 담당(에이전트) — 목업 반영 시 필수

**공통:** `.cursor/rules/workflow-confirm-before-implement.mdc` · **`.cursor/rules/design-mockup-fidelity.mdc`** (alwaysApply)

> **원칙:** 디자인이 제공된 화면은 **디자인과 동일하게** 구현한다. 임의로 작업하지 않는다.

| 순서 | 할 일 |
|------|--------|
| 1 | 목업·사용자 스크린샷 = **유일한 디자인 SSOT**. 비슷한 기존 화면·MUI·추측으로 채우지 않는다. |
| 2 | **코드 수정 전** 변경 파일·마크업·**넣지 않을 것**을 짧게 공유한다. |
| 3 | 사용자 **컨펌 후**에만 패치한다. |
| 4 | 목업에 **없는** UI·스타일(배경, shadow, 여분 버튼, 잘못된 아이콘 등)을 **임의 추가하지 않는다**. |
| 5 | 목업에 **있는** 요소(구분선, 아이콘 순서, 배경 없음 등)를 **빼거나 바꾸지 않는다**. |
| 6 | 「맞췄다」고 하기 전에 목업과 **항목별 대조**한다. |

**프롬프트 관리 상세** 진행 중이면 추가로 읽기:

- [mockup-checklist-prompt-management-redesign.md](./mockup-checklist-prompt-management-redesign.md) — §0 실수 방지 · §2 확정 스펙
- `.cursor/rules/handoff-prompt-no-mui.mdc` — MUI 제거·KL 패턴
