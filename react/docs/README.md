...
| 파일 | 용도 |
|------|------|
| **[kl-ui-guide.md](./kl-ui-guide.md)** | **UI 설계·사용 가이드 전체** — 토큰, 폼, 버튼, 모달, 목록, 표, 툴팁, 이관 규칙, 파일럿. 구현·리뷰 시 **먼저 읽기**. |
...
- **CSS 구조(Map 마더):** `.cursor/rules/kl-css-architecture.mdc` · 전역 진입 `react/src/styles/global.scss` (`tokens/` · `kit/` …)
