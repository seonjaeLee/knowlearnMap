...
import { workspaceApi, ontologyApi, reportApi, structuredApi, dictionaryApi, sllmApi } from '../services/api';
...
function NotebookDetail() {
    ...
    // --- State: LLM / sLLM Studio Config (UI only) ---
    const [llmMode, setLlmMode] = useState('LLM');           // 'LLM' or 'sLLM'
    const [llmTemperature, setLlmTemperature] = useState('0.7');
    const [sllmModels, setSllmModels] = useState([]);          // [{code, label}, ...] SLLM_MODELS active 목록
    const [selectedSllmCode, setSelectedSllmCode] = useState(''); // 사용자 선택 sLLM code
    ...
    useEffect(() => {
        ...
        // Load sLLM active models for studio panel selector
        (async () => {
            try {
                const list = await sllmApi.getActiveModels();
                if (Array.isArray(list) && list.length > 0) {
                    setSllmModels(list);
                    setSelectedSllmCode(list[0].code); // 첫 항목 자동 선택
                } else {
                    setSllmModels([]);
                    setSelectedSllmCode('');
                }
            } catch (err) {
                console.warn('sLLM 목록 조회 실패:', err);
                setSllmModels([]);
                setSelectedSllmCode('');
            }
        })();
    ...
    const handleSendMessage = async (e, textOverride) => {
        ...
        try {
            ...
            // llmTemperature 검증 (0.0-2.0 범위, 잘못된 값이면 0.7 fallback)
            let parsedTemp = parseFloat(llmTemperature);
            if (isNaN(parsedTemp) || parsedTemp < 0 || parsedTemp > 2) {
                parsedTemp = 0.7;
            }

            // llmMode 형식: 'LLM' 또는 'SLLM:<code>' (백엔드 새 분기). 코드 비어있으면 구버전 호환 'sLLM' 전송.
            const resolvedLlmMode = llmMode === 'LLM'
                ? 'LLM'
                : (selectedSllmCode ? `SLLM:${selectedSllmCode}` : 'sLLM');
            const response = await chatApi.send(id, msgText, documentIdsToSend, activePersonaText, {
                llmMode: resolvedLlmMode,
                ...
                <div className={`panel panel-right ${rightSidebarOpen ? '' : 'collapsed'}`}>
                    ...
                    {rightSidebarOpen && (
                        <div className="panel-body">
                            ...
                            {/* LLM / sLLM 선택 + Temperature (UI 만, 백엔드 연결 없음) */}
                            <div className="studio-llm-config">
                                <div className="studio-llm-mode-row">
                                    <label className="studio-llm-radio">
                                        <input
                                            type="radio"
                                            name="llm-mode"
                                            value="LLM"
                                            checked={llmMode === 'LLM'}
                                            onChange={() => setLlmMode('LLM')}
                                        />
                                        <span>LLM</span>
                                    </label>
                                    <label className="studio-llm-radio">
                                        <input
                                            ...
                                            name="llm-mode"
                                            value="sLLM"
                                            checked={llmMode === 'sLLM'}
                                            onChange={() => setLlmMode('sLLM')}
                                            disabled={sllmModels.length === 0}
                                        />
                                        <span>sLLM</span>
                                    </label>
                                </div>
                                {llmMode === 'LLM' && (
                                    <div className="studio-llm-model-row">
                                        <span className="studio-llm-model-label">모델</span>
                                        <span className="studio-llm-model-value">GEMINI 2.5 Pro</span>
                                    </div>
                                )}
                                {llmMode === 'sLLM' && (
                                    <div className="studio-llm-model-row">
                                        <span className="studio-llm-model-label">모델</span>
                                        {sllmModels.length === 0 ? (
                                            <span className="studio-llm-model-value">(등록된 sLLM 없음)</span>
                                        ) : sllmModels.length === 1 ? (
                                            <span className="studio-llm-model-value">{sllmModels[0].label}</span>
                                        ) : (
                                            <select
                                                className="studio-llm-model-select"
                                                value={selectedSllmCode}
                                                onChange={(e) => setSelectedSllmCode(e.target.value)}
                                            >
                                                {sllmModels.map(m => (
                                ...
                                <div className="studio-llm-temp-row">
                                    <label className="studio-llm-temp-label" htmlFor="studio-llm-temp-input">
                                        Temperature
                                    </label>
                                    <input
                                        id="studio-llm-temp-input"
                                        type="number"
                                        className="studio-llm-temp-input"
                                        min="0"
                                        max="2"
                                        step="0.1"
