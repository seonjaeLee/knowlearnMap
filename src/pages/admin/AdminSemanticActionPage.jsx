...
import {
  Zap, Plus, Pencil, Trash2, Save, X, Download, Upload, RotateCcw, FileDown,
...
/**
 * 전체 시멘틱 Action 관리 — 좌(액션 카테고리 type=ACTION) + 우(액션 목록).
 */
function AdminSemanticActionPage({ compact = false }) {
  ...
  const [leftExpanded, setLeftExpanded] = useState(() => {
    const stored = localStorage.getItem('admin_semantic_action_left_expanded');
  ...
  const setLeftExpandedPersist = (val) => {
    setLeftExpanded(val);
    localStorage.setItem('admin_semantic_action_left_expanded', String(val));
  };
  ...
  const fetchItems = async () => {
    try {
      ...
      const [acts, cats] = await Promise.all([
        adminSemanticApi.listActions(),
        adminSemanticApi.listCategories('ACTION'),
      ]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing.id) {
        await adminSemanticApi.updateAction(editing.id, body);
        showAlert('수정되었습니다.');
      } else {
        await adminSemanticApi.createAction(body);
        showAlert('생성되었습니다.');
      }
      setEditing(null);
      fetchItems();
  ...
  const handleDelete = async (item) => {
    const ok = await showConfirm(`"${item.nameEn} (${item.nameKo})" Action 을 삭제하시겠습니까?`);
    if (!ok) return;
    try {
      await adminSemanticApi.deleteAction(item.id);
      showAlert('삭제되었습니다.');
      fetchItems();
    } catch (err) {
  ...
  const handleExport = async () => {
    try { await adminSemanticApi.exportActions(); }
    catch (err) { showAlert('다운로드 실패: ' + (err?.message || '알 수 없는 오류')); }
  };
  const handleTemplate = async () => {
    try { await adminSemanticApi.templateActions(); }
  ...
  const handleImportFile = async (e) => {
    ...
    try {
      setImporting(true);
      const result = await adminSemanticApi.importActions(file, replace);
  ...
  return (
    <div className={compact ? '' : 'admin-page'}>
      {!compact && (
        <AdminPageHeader icon={Zap} title="액션 & 카테고리" count={items.length}
          subtitle="좌측에서 액션 카테고리를 선택해 우측 액션 목록을 필터링합니다." />
      )}
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
        {/* 좌: 액션 카테고리 (type=ACTION) */}
        <div style={{
          ...
          {loading ? (
            ...
            <div className="admin-table-wrap" style={{ maxHeight: '62vh', overflowY: 'auto' }}>
              <table className="admin-table">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={6}><div className="admin-empty-state" style={{ border: 'none', padding: '32px 0' }}><p className="admin-empty-state-title">표시할 Action 이 없습니다</p></div></td></tr>
                  ) : (
                    filtered.map((item) => (
                      <tr key={item.id}>
        ...
        <div className="admin-modal-overlay" onClick={() => setEditing(null)}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            <div className="admin-modal-header">
              <h3 className="admin-modal-title">{editing.id ? 'Action 수정' : 'Action 추가'}</h3>
            ...
            <div className="admin-modal-body">
              <div className="admin-field">
                ...
                <input className="admin-input" value={editing.nameEn || ''}
                  onChange={(e) => setEditing({ ...editing, nameEn: e.target.value })} placeholder="e.g. ReorderStock" />
              </div>
              <div className="admin-field">
                <label className="admin-field-label">한글명 *</label>
                <input className="admin-input" value={editing.nameKo || ''}
                  onChange={(e) => setEditing({ ...editing, nameKo: e.target.value })} placeholder="예: 재발주" />
              </div>
              <div className="admin-field">
