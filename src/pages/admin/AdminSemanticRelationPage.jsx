...
import {
  Link2, Plus, Pencil, Trash2, Save, X, Download, Upload, RotateCcw, FileDown,
...
function AdminSemanticRelationPage({ compact = false }) {
  ...
  const handleDelete = async (item) => {
    const ok = await showConfirm(`"${item.nameEn} (${item.nameKo})" 관계를 삭제하시겠습니까?`);
    if (!ok) return;
    try {
          ...
          {loading ? (
            ...
            <div className="admin-table-wrap" style={{ maxHeight: '62vh', overflowY: 'auto' }}>
              <table className="admin-table">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={6}><div className="admin-empty-state" style={{ border: 'none', padding: '32px 0' }}><p className="admin-empty-state-title">표시할 관계가 없습니다</p></div></td></tr>
                  ) : (
                    filtered.map((item) => (
                      <tr key={item.id}>
        ...
        <div className="admin-modal-overlay" onClick={() => setEditing(null)}>
          <div className="admin-modal" onClick={(e) => e.stopPropagation()}>
            <div className="admin-modal-header">
              <h3 className="admin-modal-title">{editing.id ? '관계 수정' : '관계 추가'}</h3>
