...
function AdminSemanticObjectPage({ compact = false }) {
  ...
  const handleDelete = async (item) => {
    const ok = await confirm(`"${item.nameEn} (${item.nameKo})" Object 를 삭제하시겠습니까?`);
    if (!ok) return;
    try {
          ...
          {loading ? (
            ...
            <div className="admin-table-wrap admin-semantic-table-wrap">
              <table className="admin-table">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr><td colSpan={6}><div className="admin-empty-state admin-empty-state-default"><p className="admin-empty-state-title">표시할 Object 가 없습니다</p></div></td></tr>
                  ) : (
                    filtered.map((item) => (
                      <tr key={item.id}>
